...
-- Drop all referencing tables first (children)
...
-- Then the core content table
DROP TABLE IF EXISTS Content;

-- Now drop all referenced (parent) tables
...
-- Drop subscription/payment/user core tables last
...
CREATE TABLE Content_Availability (
    ...
    region VARCHAR(100) NOT NULL,      -- e.g., US, Global, Europe
    status ENUM('available', 'unavailable') DEFAULT 'available'
);

CREATE TABLE Content_Format (
    format_id INT PRIMARY KEY AUTO_INCREMENT,
    format_type VARCHAR(50) NOT NULL  -- e.g., HD, 4K, SD
);
...
CREATE TABLE Actor_Content (
    content_id INT,
    actor_id INT,
    actor_role VARCHAR(100), -- realistic
